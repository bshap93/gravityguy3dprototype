    ...
    public class PlayerInput_InputSystem_SpaceshipControls : VehicleInput
    {
        ...
        protected void UpdateReticulePosition(Vector3 mouseDelta)
        {
            if (mouseSteeringType == MouseSteeringType.ScreenPosition)
            {
                ...
                Vector3 delta = new Vector3(mouseDelta.x / Screen.width, mouseDelta.y / Screen.height, 0);

                if (mouseVerticalInverted) delta.y *= -1;
                if (mouseHorizontalInverted) delta.x *= -1;

                // Add the delta 
                reticuleViewportPosition += delta * reticleMovementSpeed;
        ...
        protected virtual void MouseSteeringUpdate()
        {
            ...
            else if (mouseSteeringType == MouseSteeringType.DeltaPosition)
            {
                ...
                screenInputs.y *= (mouseVerticalInverted ? -1 : 1);
                screenInputs.x *= (mouseHorizontalInverted ? -1 : 1);
            }
